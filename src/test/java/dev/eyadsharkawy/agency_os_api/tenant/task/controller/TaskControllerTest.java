...
public class TaskControllerTest {
    ...
    void testCreateTask_Success() throws Exception {
        TaskRequest request = new TaskRequest("Task Name", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.TODO, UUID.randomUUID(), Set.of("assigneeId"));
        TaskResponse response = new TaskResponse(UUID.randomUUID(), "Task Name", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.TODO, UUID.randomUUID(), Set.of("assigneeId"), 0, false, Instant.now(), Instant.now());
    ...
    void testGetTaskById_Success() throws Exception {
        ...
        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(taskService.getTaskById(taskId)).thenReturn(new TaskResponse(taskId, "Task", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.TODO, UUID.randomUUID(), Set.of("user"), 0, false, Instant.now(), Instant.now()));
    ...
    void testUpdateTask_Success() throws Exception {
        UUID taskId = UUID.randomUUID();
        TaskRequest request = new TaskRequest("Updated Task", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.IN_PROGRESS, UUID.randomUUID(), Set.of("assigneeId"));

        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(taskService.updateTaskById(any(), any())).thenReturn(new TaskResponse(taskId, "Updated Task", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.IN_PROGRESS, UUID.randomUUID(), Set.of("assigneeId"), 0, false, Instant.now(), Instant.now()));
    ...
    void testUpdateTaskStatus_Success() throws Exception {
        ...
        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(taskService.updateTaskStatus(any(), any())).thenReturn(new TaskResponse(taskId, "Updated Task", "Desc", Instant.now(), Instant.now(), 120, TaskPriority.HIGH, TaskStatus.DONE, UUID.randomUUID(), Set.of("assigneeId"), 0, false, Instant.now(), Instant.now()));
