...
class ClientControllerTest {
  ...
  void testCreateClient_Success() throws Exception {
    ClientRequest request =
        new ClientRequest("Client Name", "test@client.com", ClientStatus.ACTIVE);
  ...
  void testUpdateClientById_Success() throws Exception {
    UUID clientId = UUID.randomUUID();
    ClientRequest request =
        new ClientRequest("Updated Client Name", "updated@client.com", ClientStatus.INACTIVE);

    when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
    when(clientService.updateClientById(any(), any()))
        .thenReturn(
            new ClientResponse(
                clientId,
                "Updated Client Name",
    ...
    mockMvc
        .perform(
            put("/api/v1/clients/{id}", clientId)
                .header("X-Tenant-ID", "tenant1")
                .with(SecurityMockMvcRequestPostProcessors.jwt())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
