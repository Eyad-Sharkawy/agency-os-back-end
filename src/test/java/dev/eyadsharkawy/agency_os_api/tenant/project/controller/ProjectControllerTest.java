...
public class ProjectControllerTest {
    ...
    void testCreateProject_Success() throws Exception {
        ProjectRequest request = new ProjectRequest("Project Name", BigDecimal.valueOf(1000), ProjectStatus.IN_PROGRESS, UUID.randomUUID(), BigDecimal.valueOf(100));
        ProjectResponse response = new ProjectResponse(UUID.randomUUID(), "Project Name", BigDecimal.valueOf(1000), ProjectStatus.IN_PROGRESS, UUID.randomUUID(), BigDecimal.valueOf(100), Instant.now(), Instant.now());
    ...
    void testGetProjectById_Success() throws Exception {
        ...
        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(projectService.getProjectById(projectId)).thenReturn(new ProjectResponse(projectId, "Project", BigDecimal.TEN, ProjectStatus.IN_PROGRESS, UUID.randomUUID(), BigDecimal.TEN, Instant.now(), Instant.now()));
    ...
    void testGetProjectsByClientId_Success() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/projects/client/{clientId}", clientId)
                        .header("X-Tenant-ID", "tenant1")
                        .with(SecurityMockMvcRequestPostProcessors.jwt()))
                .andExpect(status().isOk());
    }
    ...
    void testUpdateProject_Success() throws Exception {
        UUID projectId = UUID.randomUUID();
        ProjectRequest request = new ProjectRequest("Updated Name", BigDecimal.valueOf(2000), ProjectStatus.DELIVERED, UUID.randomUUID(), BigDecimal.valueOf(100));

        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(projectService.updateProjectById(any(), any())).thenReturn(new ProjectResponse(projectId, "Updated Name", BigDecimal.valueOf(2000), ProjectStatus.DELIVERED, UUID.randomUUID(), BigDecimal.valueOf(100), Instant.now(), Instant.now()));
