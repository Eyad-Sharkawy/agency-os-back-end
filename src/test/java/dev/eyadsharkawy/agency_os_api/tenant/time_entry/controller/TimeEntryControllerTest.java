...
class TimeEntryControllerTest {
  ...
  void testStartTimer_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/time-entries/start/{taskId}", taskId)
                .header("X-Tenant-ID", "tenant1")
                .with(SecurityMockMvcRequestPostProcessors.jwt()))
        .andExpect(status().isCreated());
  }
  ...
  void testGetTimeEntriesByTaskId_Success() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/time-entries/task/{taskId}", taskId)
                .header("X-Tenant-ID", "tenant1")
                .with(SecurityMockMvcRequestPostProcessors.jwt()))
        .andExpect(status().isOk());
  }
  ...
  void testGetTimeEntriesByUserId_Success() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/time-entries/user/{userId}", userId)
                .header("X-Tenant-ID", "tenant1")
                .with(SecurityMockMvcRequestPostProcessors.jwt()))
        .andExpect(status().isOk());
  }
  ...
  void testDeleteTimeEntry_Success() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/v1/time-entries/{id}", entryId)
                .header("X-Tenant-ID", "tenant1")
                .with(SecurityMockMvcRequestPostProcessors.jwt()))
