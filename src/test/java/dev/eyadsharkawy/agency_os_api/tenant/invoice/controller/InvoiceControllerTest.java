...
public class InvoiceControllerTest {
    ...
    void testCreateInvoice_Success() throws Exception {
        ...
        InvoiceResponse response = new InvoiceResponse(UUID.randomUUID(), UUID.randomUUID(), BigDecimal.valueOf(100), InvoiceStatus.DRAFT, Instant.now(), Instant.now());
    ...
    void testGetInvoiceById_Success() throws Exception {
        ...
        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(invoiceService.getInvoiceById(invoiceId)).thenReturn(new InvoiceResponse(invoiceId, UUID.randomUUID(), BigDecimal.valueOf(100), InvoiceStatus.DRAFT, Instant.now(), Instant.now()));
    ...
    void testGetInvoicesByClientId_Success() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/invoices/client/{clientId}", clientId)
                        .header("X-Tenant-ID", "tenant1")
                        .with(SecurityMockMvcRequestPostProcessors.jwt()))
                .andExpect(status().isOk());
    }
    ...
    void testUpdateInvoice_Success() throws Exception {
        ...
        when(workspaceSecurity.hasRole(any(String[].class))).thenReturn(true);
        when(invoiceService.updateInvoiceById(any(), any())).thenReturn(new InvoiceResponse(invoiceId, UUID.randomUUID(), BigDecimal.valueOf(100), InvoiceStatus.SENT, Instant.now(), Instant.now()));
