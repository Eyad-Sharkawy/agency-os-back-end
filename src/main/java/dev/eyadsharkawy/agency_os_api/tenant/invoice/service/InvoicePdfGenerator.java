...
public class InvoicePdfGenerator {
    ...
    public static byte[] generate(Invoice invoice, String agencyName, String contactEmail, List<TimeEntry> billedEntries) throws IOException {
        try (PDDocument document = new PDDocument()) {
            ...
            contentStream.showText(invoice.getClient().getEmail() != null ? invoice.getClient().getEmail() : "N/A");
            contentStream.endText();

            // Column 2: PROJECT
            String projectName = "General Billing";
            if (billedEntries != null && !billedEntries.isEmpty()) {
                long uniqueProjects = billedEntries.stream()
            ...
            contentStream.showText(invoice.getCreatedAt().atZone(ZoneOffset.UTC).format(DATE_FORMATTER).substring(0, 10));
            contentStream.newLineAtOffset(0, -15);
            contentStream.showText(dueDate.atZone(ZoneOffset.UTC).format(DATE_FORMATTER).substring(0, 10));
            contentStream.endText();
            ...
            for (TimeEntry entry : billedEntries) {
                ...
                BigDecimal entryCost = BigDecimal.valueOf(hoursVal).multiply(billingRate).setScale(2, RoundingMode.HALF_UP);
                ...
                String itemDesc = String.format("%s - %s (%.1f hrs)", projectNameVal, taskTitleVal, hoursVal);
            ...
            // If there's not enough room left on the current page to fit the totals and terms (requires ~130pt)
            if (yPosition < 130) {
                contentStream.close();
