...
public class TimeEntryService {
    ...
    public TimeEntryResponse logTimeManually(Jwt jwt, TimeEntryRequest request) {
        ...
        log.info("User [{}] manually logging [{}] minutes on task [{}]", userId, request.durationMinutes(), request.taskId());
    ...
    public TimeEntryResponse stopTimer(Jwt jwt, boolean isBillable) {
        ...
        ActiveTimer activeTimer = activeTimerRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("No active timer found for this user"));

        int durationMinutes = Math.max(1, (int) Duration.between(activeTimer.getStartTime(), Instant.now()).toMinutes());
    ...
    public Optional<ActiveTimerResponse> getActiveTimer(Jwt jwt) {
        String userId = jwt.getSubject();
        return activeTimerRepository.findById(userId)
                .map(ActiveTimerResponse::fromEntity);
    }
