...
@Tag(name = "06. Time Tracking", description = "Endpoints for manual time logging, start/stop stopwatch timers, and WebSocket broadcast integrations")
public class TimeEntryController {
    ...
    @PostMapping
    @Operation(summary = "Log time manually", description = "Allows internal users to log a manual time entry for a task. Broadcasts updates to the active tenant channel via WebSockets.")
    ...
    @Operation(summary = "Start stopwatch timer", description = "Initiates a live stopwatch timer on a specific task. If another timer is running, it will automatically stop it first.")
    ...
    @PostMapping("/stop")
    @Operation(summary = "Stop stopwatch timer", description = "Stops the currently running stopwatch timer for the user, converting the active duration into a logged time entry.")
    public ResponseEntity<TimeEntryResponse> stopTimer(
            @AuthenticationPrincipal Jwt jwt,
            @Parameter(description = "Whether this tracked entry is billable to the client") @RequestParam(defaultValue = "true") boolean isBillable) {
    ...
    @GetMapping("/active")
    @Operation(summary = "Get user's active timer", description = "Retrieves the active running stopwatch timer metadata for the authenticated user, or returns 204 No Content if none is running.")
    ...
    @Operation(summary = "Get time entries by Task", description = "Retrieves the list of all logged time entries associated with a specific task.")
    ...
    @Operation(summary = "Get time entries by User", description = "Retrieves the list of all logged time entries submitted by a specific user ID.")
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete time entry", description = "Deletes a logged time entry from the timesheet history.")
