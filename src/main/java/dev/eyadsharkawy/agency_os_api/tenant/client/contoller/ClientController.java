...
@Tag(name = "03. Clients", description = "Endpoints for managing client companies and customer accounts within the tenant space")
public class ClientController {
    ...
    @Operation(summary = "Create client company", description = "Registers a new client company within the current workspace tenant. Restricted to OWNER or ADMIN.")
    ...
    @Operation(summary = "List all clients", description = "Retrieves all registered client companies in the active tenant. Restricted to OWNER, ADMIN, or MEMBER.")
    ...
    @GetMapping("/{id}")
    @PreAuthorize("@workspaceSecurity.hasRole('OWNER', 'ADMIN', 'MEMBER')")
    @Operation(summary = "Get client by ID", description = "Retrieves metadata of a specific client company by its unique identifier. Restricted to OWNER, ADMIN, or MEMBER.")
    ...
    @Operation(summary = "Update client details", description = "Modifies registration and profile details of a client company. Restricted strictly to the OWNER.")
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("@workspaceSecurity.hasRole('OWNER')")
    @Operation(summary = "Delete client company", description = "Deletes a client company from the active tenant space. Restricted strictly to the OWNER.")
