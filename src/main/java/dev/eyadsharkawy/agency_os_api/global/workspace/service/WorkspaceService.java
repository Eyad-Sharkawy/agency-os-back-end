...
public class WorkspaceService {
  ...
  public WorkspaceResponse createWorkspace(Jwt jwt, WorkspaceRequest request) {
    ...
    log.info(
        "Workspace [{}] committed with schema [{}] pending provisioning",
        workspace.getName(),
        tenantId);
  ...
  public List<WorkspaceResponse> getUserWorkspaces(Jwt jwt) {
    ...
    return userRepository
        ...
        .map(
            user ->
                user.getUserWorkspaces().stream()
                    .map(
                        userWorkspace ->
                            WorkspaceResponse.fromEntity(
                                userWorkspace.getWorkspace(), userWorkspace.getRole()))
                    .toList())
        .orElse(List.of());
  }
  ...
  @Transactional
  public WorkspaceResponse updateUserWorkspaceByTenantId(
      String tenantId, WorkspaceRequest workspaceRequest) {
    Workspace workspace = findWorkspaceByTenantIdOrThrow(tenantId);
  ...
  @Transactional
  public void updateWorkspaceMember(
      Jwt jwt, String tenantId, UUID userId, WorkspaceMemberUpdateRequest request) {
    Workspace workspace = findWorkspaceByTenantIdOrThrow(tenantId);
  ...
  public void removeWorkspaceMember(String requesterKeycloakId, String tenantId, UUID userId) {
    ...
    UserWorkspace membership =
        userWorkspaceRepository
            .findById(new UserWorkspaceId(userId, workspace.getId()))
  ...
  public void transferOwnership(
      String currentOwnerKeycloakId, String tenantId, UUID newOwnerUserId) {
    Workspace workspace =
        workspaceRepository
            .findByTenantId(tenantId)
            .orElseThrow(() -> new ResourceNotFoundException("Workspace not found: " + tenantId));
    ...
    UserWorkspace newOwnerMembership =
        userWorkspaceRepository
            .findById(new UserWorkspaceId(newOwnerUserId, workspace.getId()))
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
    ...
    log.info(
        "Ownership of workspace [{}] transferred from [{}] to [{}]",
        tenantId,
  ...
  private Workspace findWorkspaceByTenantIdOrThrow(String tenantId) {
    return workspaceRepository
        .findByTenantId(tenantId)
        .orElseThrow(
            () -> new ResourceNotFoundException("Workspace not found with tenant id: " + tenantId));
  }
  ...
  private UserWorkspace findUserWorkspaceByIdOrThrow(UserWorkspaceId id) {
    return userWorkspaceRepository
        ...
        .orElseThrow(
            () ->
                new ResourceNotFoundException("Membership not found for user in this workspace."));
  }
}
