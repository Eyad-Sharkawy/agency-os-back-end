...
public class WorkspaceInvitationService {
    ...
    @Transactional
    public WorkspaceInvitationResponse inviteUser(Jwt inviterJwt, String tenantId, WorkspaceInvitationRequest request) {
        Workspace workspace = workspaceRepository.findByTenantId(tenantId)
                .orElseThrow(() -> new ResourceNotFoundException("Workspace not found with Id: " + tenantId));

        AppUser invitee;

        if (request.username() != null && !request.username().isBlank())
            invitee = userRepository.findByUsername(request.username())
            ...
            invitee = userRepository.findByEmail(request.email())
                    .orElseThrow(() -> new ResourceNotFoundException("User not found with username: " + request.email()));

        else
            throw new IllegalArgumentException("Either username or username must be provided to invite a user.");

        if (request.role() != WorkspaceRole.MEMBER) {
            String inviterKeycloakId = inviterJwt.getSubject();
            WorkspaceRole inviterRole = userWorkspaceRepository.findRoleByKeycloakIdAndTenantId(inviterKeycloakId, tenantId)
                    .orElseThrow(() -> new AccessDeniedException("Access Denied: Inviter is not a member of this workspace."));
        ...
        boolean isMember = userWorkspaceRepository.findRoleByKeycloakIdAndTenantId(invitee.getKeycloakId(), tenantId).isPresent();

        if (isMember)
            throw new IllegalArgumentException("User is already a member of this workspace.");

        invitationRepository.findByWorkspaceIdAndUsernameIgnoreCase(workspace.getId(), invitee.getUsername())
        ...
        log.info("Invitation created for username [{}] in workspace [{}] by [{}]", invitee.getUsername(), tenantId, inviterUsername);
    ...
    public List<WorkspaceInvitationResponse> getPendingInvitations(Jwt jwt) {
        ...
        return invitationRepository.findByUsernameIgnoreCaseAndStatus(user.getUsername(), InvitationStatus.PENDING)
                .stream()
    ...
    public void acceptInvitation(Jwt jwt, UUID invitationId) {
        ...
        if (invitation.getRole() == WorkspaceRole.CLIENT) {
            ...
            try {
                Client client = clientRepository.findById(invitation.getClientId())
                        .orElseThrow(() -> new ResourceNotFoundException("Client not found with id: " + invitation.getClientId()));
        ...
        log.info("User [{}] accepted invitation for workspace [{}]", user.getUsername(), invitation.getWorkspace().getTenantId());
    }
    ...
    public void declineInvitation(Jwt jwt, UUID invitationId) {
        ...
        invitationRepository.save(invitation);
        log.info("User [{}] declined invitation for workspace [{}]", user.getUsername(), invitation.getWorkspace().getTenantId());
    }
}
