...
public class WebSocketAuthChannelInterceptor implements ChannelInterceptor {
    ...
    private static final Pattern TENANT_TOPIC_PATTERN = Pattern.compile("^/topic/(tenant_[a-z0-9_]+)/.*");
    ...
    private void authorizeSubscriptionFrame(StompHeaderAccessor accessor) {
        ...
        if (matcher.matches()) {
            ...
            log.debug("User [{}] attempting to subscribe to destination [{}] for tenant [{}]", keycloakUserId, destination, tenantId);

            boolean isAuthorized = workspaceRepository.isUserMemberOfTenant(keycloakUserId, tenantId);
            if (!isAuthorized) {
                log.warn("WebSocket SUBSCRIBE rejected: User [{}] is not a member of tenant [{}]", keycloakUserId, tenantId);
                throw new IllegalArgumentException("Access denied to tenant topic: " + tenantId);
            }

            log.info("WebSocket SUBSCRIBE authorized for user [{}] to tenant topic [{}]", keycloakUserId, destination);
        }
    }
}
